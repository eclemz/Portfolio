      ...
      <div className=" w-full xl:w-[900px] lg:w-[850px] md:w-[600px] grid lg:grid-cols-3 grid-cols-1 lg:gap-12 gap-32 xl:mb-0 mb-16">
        {skillCards.map((card, index) => (
          <div
            ...
            onClick={() => setHoveredCardIndex(hoveredCardIndex === index ? null : index)}
            onMouseEnter={() => setHoveredCardIndex(index)}
            onMouseLeave={() => setHoveredCardIndex(null)}
          >
            <div className="p-3 bg-gray-200 dark:bg-gray-800 transition-colors duration-500">
              <i
                className={`${card.icon} md:text-4xl text-3xl text-gray-900 dark:text-white transition-colors duration-500 mb-0`}
              ></i>

              <h3 className="md:text-2xl text-xl font-bold my-4 txt-red-500 dark:text-yellow-500 transition-colors duration-500">
                {card.title}
              </h3>
            ...
            <div
              className={`w-full absolute left-0 ${getPositionClass(card)} 
